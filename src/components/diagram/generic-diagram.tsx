  ...
  if (error) {
    return (
      <div className="text-center py-8">
        ...
        <pre className="mt-4 p-4 text-[10px] text-left overflow-auto bg-zinc-50 rounded-lg font-mono text-zinc-700 ">{mermaidSyntax}</pre>
      </div>
    );
  }

  return (
    <div className={`${isFullscreen ? 'fixed inset-0 z-[100] rounded-none border-0' : 'rounded-[24px] border border-zinc-200 relative group'} bg-white overflow-hidden flex flex-col`}>
      {!isRendered && (
            ...
            <>
              ...
              <div className="absolute top-4 left-4 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="flex items-center rounded-lg border border-zinc-200 bg-white overflow-hidden">
                  <div className="px-3 py-1.5 text-xs text-zinc-500 bg-zinc-50 border-r border-zinc-200 flex items-center gap-1.5">
                    {isDownloading ? (
                  ...
                  <button onClick={() => handleDownload('png')} disabled={isDownloading} className="px-3 py-1.5 text-xs font-medium text-zinc-700 border-r border-zinc-200 hover:bg-zinc-50 :bg-zinc-700 disabled:opacity-50 transition-colors">PNG</button>
                  <button onClick={() => handleDownload('pdf')} disabled={isDownloading} className="px-3 py-1.5 text-xs font-medium text-zinc-700 border-r border-zinc-200 hover:bg-zinc-50 :bg-zinc-700 disabled:opacity-50 transition-colors">PDF</button>
                  <button onClick={() => handleDownload('svg')} disabled={isDownloading} className="px-3 py-1.5 text-xs font-medium text-zinc-700 hover:bg-zinc-50 :bg-zinc-700 disabled:opacity-50 transition-colors">SVG</button>
                </div>
              </div>

              {/* Zoom Controls Overlay (Top Right) */}
              <div className="absolute top-4 right-4 z-10 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => zoomIn()} className="p-2 bg-white text-zinc-600 border border-zinc-200 rounded-lg hover:bg-zinc-50 :bg-zinc-700 transition-colors" title="Zoom In">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
                </button>
                <button onClick={() => zoomOut()} className="p-2 bg-white text-zinc-600 border border-zinc-200 rounded-lg hover:bg-zinc-50 :bg-zinc-700 transition-colors" title="Zoom Out">
                ...
                <button onClick={() => resetTransform()} className="p-2 bg-white text-zinc-600 border border-zinc-200 rounded-lg hover:bg-zinc-50 :bg-zinc-700 transition-colors" title="Reset View">
                ...
                <button onClick={() => setIsFullscreen(!isFullscreen)} className="p-2 bg-white text-zinc-600 border border-zinc-200 rounded-lg hover:bg-zinc-50 :bg-zinc-700 transition-colors" title={isFullscreen ? "Exit Fullscreen" : "Fullscreen"}>
                  {isFullscreen ? (
