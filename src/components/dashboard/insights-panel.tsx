    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8 animate-fade-in" style={{ animationDelay: "150ms" }}>
      ...
      <div className="rounded-2xl bg-zinc-900/50 border border-white/5 p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-semibold text-zinc-100">Due Today</h3>
        </div>
        <div className="space-y-3">
          {dueToday.length > 0 ? dueToday.map(item => (
            <div key={item.id} onClick={() => router.push(`/project/${item.id}`)} className="group flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/10 cursor-pointer transition-colors">
              <span className="text-sm font-medium text-zinc-300 truncate group-hover:text-amber-300 transition-colors">{item.title}</span>
              <span className="text-[10px] text-zinc-500 shrink-0">{item.time}</span>
            </div>
          )) : (
            <p className="text-xs text-zinc-500 py-2">No tasks due today.</p>
          )}
        </div>
      </div>
      ...
      <div className="rounded-2xl bg-zinc-900/50 border border-white/5 p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-semibold text-zinc-100">Overdue</h3>
        </div>
        <div className="space-y-3">
          {overdue.length > 0 ? overdue.map(item => (
            <div key={item.id} onClick={() => router.push(`/project/${item.id}`)} className="group flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/10 cursor-pointer transition-colors">
              <span className="text-sm font-medium text-zinc-300 truncate group-hover:text-red-300 transition-colors">{item.title}</span>
              <span className="text-[10px] text-red-500 shrink-0">{item.time}</span>
            </div>
          )) : (
            <p className="text-xs text-zinc-500 py-2">No overdue projects. Great job!</p>
          )}
        </div>
      </div>

      {/* Recent Activity */}
      <div className="rounded-2xl bg-zinc-900/50 border border-white/5 p-5 md:col-span-2 lg:col-span-1">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-semibold text-zinc-100">Recently Updated</h3>
        </div>
        <div className="space-y-3">
          {recentActivity.map(item => (
            <div key={item.id} onClick={() => router.push(`/project/${item.id}`)} className="group flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/10 cursor-pointer transition-colors">
              <span className="text-sm font-medium text-zinc-300 truncate group-hover:text-blue-300 transition-colors">{item.title}</span>
              <span className="text-[10px] text-zinc-500 shrink-0 tabular-nums">{item.time.split(' ')[0]}</span>
            </div>
          ))}
        </div>
      </div>
