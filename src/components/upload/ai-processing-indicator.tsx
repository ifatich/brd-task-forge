...
export function AiProcessingIndicator({
  ...
  return (
    <div className="rounded-[24px] border border-hairline p-6 bg-canvas animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="text-right">
          <div className="text-3xl font-bold text-ink tabular-nums font-mono">
            {progressPercent}%
          </div>
      ...
      {/* Main Progress Bar */}
      <div className="relative h-2 rounded-full bg-black/5 overflow-hidden mb-10">
        <div
        ...
        >
           {!isError && !isDone && (
              <div className="absolute inset-0 bg-canvas/20 animate-[shimmer_2s_infinite] -skew-x-12" />
           )}
        </div>
      </div>
      ...
      <div className="space-y-6 relative mb-8">
        <div className="absolute top-2 bottom-2 left-[11px] w-px bg-zinc-200 " />
        ...
        {STAGES.map((stage, idx) => {
          ...
          return (
            <div key={stage.id} className="relative flex gap-4">
              <div className="relative z-10 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-canvas border-2 transition-colors duration-300">
                {isPassed ? (
                  <div className="h-full w-full rounded-full bg-green-500 border-2 border-white flex items-center justify-center">
                  ...
                  <div className="h-full w-full rounded-full bg-red-500 border-2 border-white flex items-center justify-center">
              ...
              <div className="flex-1 pb-2">
                <h4 className={`text-sm font-semibold transition-colors duration-300 ${isActive || isPassed ? 'text-ink ' : 'text-ink/40 '}`}>
                  {stage.label}
                </h4>
                
                {/* Rotating Messages */}
                <div className={`mt-1.5 overflow-hidden transition-all duration-500 ${isActive ? 'max-h-12 opacity-100' : 'max-h-0 opacity-0'}`}>
                  <p className="text-xs text-ink/60 italic animate-fade-in flex items-center gap-2">
      ...
      {/* Footer Actions */}
      <div className="flex flex-col sm:flex-row items-center gap-3 pt-6 border-t border-hairline ">
        {!isDone && !isError && (
          <button
            onClick={() => router.push("/")}
            className="w-full sm:w-auto inline-flex items-center justify-center px-5 py-2.5 rounded-[24px] border border-hairline bg-surface-soft hover:bg-black/5 :bg-zinc-700 text-sm font-medium text-ink/80 transition-colors"
          >
        ...
        {isError && onRetry && (
          <button
            onClick={onRetry}
            className="w-full sm:w-auto inline-flex items-center justify-center px-5 py-2.5 rounded-[24px] bg-red-600 hover:bg-red-700 text-sm font-medium text-white transition-colors"
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-2">
